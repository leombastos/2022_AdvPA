# Assignment #4 - Focal filter  
In class, we have used a focal filter to smooth out the zones assignment for our field.  

Now, you are asked to play around with the focal filter arguments of matrix size and function type, and answer questions based on the results.  
...
For each of these three new scenarios, create a map with the result making sure the map title (tip: in function `labs()`) reflects the matrix size and function type, and export this plot to your "output" folder in a .png format, following the name scheme of zonesmoothed_matrixsize_function (e.g., zonesmoothed_5x5_mean.png for the settings using during class). 

2. Copy into a word file the zone maps:  
- Before smoothing (done in class)  
- Matrix 5 x 5, smooth function of mean (done in class)
- Each of the three scenarios ran above  

After pasting all maps on word, resize each of them to be 1.56 inch height x 1.17 inch width (so you can see all of them in one row). After this step, answer the following questions:  
